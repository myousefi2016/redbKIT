...
% Dirichlet
data.bcDir{1,7} = @(x, y, z, t, param)( data.bcDir_t(t)  * N1(1) * 1 + 0.*x.*y); 
data.bcDir{2,7} = @(x, y, z, t, param)( data.bcDir_t(t)  * N1(2) * 1 + 0.*x.*y); 
data.bcDir{3,7} = @(x, y, z, t, param)( data.bcDir_t(t)  * N1(3) * 1 + 0.*x.*y); 

data.bcDir{1,2} = @(x, y, z, t, param)(0.*x.*y); 
data.bcDir{2,2} = @(x, y, z, t, param)(0.*x.*y); 
data.bcDir{3,2} = @(x, y, z, t, param)(0.*x.*y);
...
% flags
data.flag_dirichlet{1} = [2 7];

data.flag_dirichlet{2} = [2 7];

data.flag_dirichlet{3} = [2 7];

data.flag_neumann{1}   = [3:6 8:10];
data.flag_neumann{2}   = [3:6 8:10];
data.flag_neumann{3}   = [3:6 8:10];
